...
flapi.prototype.authApp = function(authCallbackURL, callback){
  var self = this;
  ...
  if(!this.settings.oauth_consumer_key){
    return callback(new Error('An oauth_consumer_key (app key) is required. ' + message));
  }

  if(!this.settings.oauth_consumer_secret){
    return callback(new Error('An oauth_consumer_secret (app secret) is required. ' + message));
  }

  if(!authCallbackURL || typeof(authCallbackURL) !== 'string') {
    return callback(new Error('Please define a an auth callback url. ' + message));
  }
  ...
  var wrappedCb = function(err, body, res) {
    if (err) {
      return callback(err);
    }
    ...
    for(var key in returnObj){
      if(Object.prototype.hasOwnProperty.call(returnObj, key)){
        self.settings[key] = returnObj[key];
      }
    }

    callback(null, self.settings, res);
  };

  flickr.makeRequest(request, null, wrappedCb);
};
...
flapi.prototype.getUserAuthURL = function(){
  ...
  return 'https://' + apiHost + '/services/oauth/authorize?oauth_token=' + this.settings.oauth_token + perms;
};


// The "token" is really an object with an oauth_token and an
// oauth_token_secret unique to this individual
flapi.prototype.getUserAccessToken = function(oauth_verifier, callback){
  var request = {
    method      : 'GET',
  ...
  var wrappedCb = function(err, body, res) {
    ...
    callback(null, queryString.parse(body), res);
  };
...
flapi.prototype.api = function(opts, callback){

  if(!opts.method){
    return callback(new Error('Please pass an api method option as "method". You can find all available flickr methods within the flickr documentation at http://www.flickr.com/services/api/'));
  }
  ...
  if(!opts.preventCall){
    ...
    flickr.makeRequest(request, opts.params.photo, wrappedCb);
  }
  ...
  var earl  = 'https://' + path.join(request.host, request.path);
  return earl + '?' + query;
};

...
flickr.prototype.authApp = function(authCallbackURL, next){
  var self = this;
  var message = '\
Please visit the flickr documentation http://www.flickr.com/services/api/, or retrieve your app creds here http://www.flickr.com/services/apps, or create an app here http://www.flickr.com/services/apps/create/ \n';

  if(!this.settings.oauth_consumer_key) throw new Error('\
An oauth_consumer_key (app key) is required. ' + message);

  if(!this.settings.oauth_consumer_secret) throw new Error('\
An oauth_consumer_secret (app secret) is required. ' + message);
...
flickr.prototype.getUserAuthURL = function(){
  var perms = (this.settings.perms) ? '&perms=' + this.settings.perms : ''
  return 'http://api.flickr.com/services/oauth/authorize?oauth_token=' + this.settings.oauth_token + perms
};


// The "token" is really an object with an oauth_token and an
// oauth_token_secret unique to this individual
flickr.prototype.getUserAccessToken = function(oauth_verifier, next){
  var request = {
    method      : 'GET',
...
flickr.prototype.api = function(opts){

  if(!opts.method) throw new Error('\
Please pass an api method option as "method". You can find all available flickr methods within the flickr documentation at http://www.flickr.com/services/api/');

  var queryParams = prepareParams(this.settings, {
    method          : opts.method,
    format          : 'json',
    nojsoncallback  : 1,
    oauth_token     : opts.accessToken.oauth_token
  });

  // Any url params which are passed in should be added
  // to the query params object
  if(opts.params){
  ...
  // If a data object is attached, we know it's a post
  if(opts.data){
    request.method  = 'POST';
    request.data    = opts.data;
  }

  // If the method is a photto photo upload, we need to
  // change both the host and path
  if(opts.method == 'upload'){
    request.host = 'up.flickr.com';
    request.path = '/services/upload/';
  }

  // Create a valid signature only if an access token is passed
  if(opts.accessToken){
    request.queryParams.oauth_signature = buildSignature(request,
      this.settings.oauth_consumer_secret,
  ...
  makeRequest(request, function(body, res){
    if(opts.next){
      if(body[0] == '{' || body[0] == '[') opts.next(JSON.parse(body))
      else console.log('Error : ', res.statusCode, ' - ', body);
    };
  });
};
...
var makeRequest = function(request, next){
  ...
  var reqOpts  = {
    host    : request.host || 'api.flickr.com',
    path    : request.path + query,
    method  : request.method
  };

  // When posting, we need to pass the post data in the form instead of the query
  if(reqOpts.method == 'POST'){
    var postParams    = request.queryParams;
    postParams.data   = request.data;
  ...
  // If a photo, we need to use a multipart form
  if(reqOpts.data && reqOpts.data.photo){
  ...
  if(postData.length) httpReq.write(postData);
  httpReq.end();
};


// Convenience for adding nonce, timestamp, and version
...
var prepareParams = function(originalObj, objToAdd){
  ...
  ['consumer_secret'].forEach(function(banned){
    delete obj[banned]
  });
...
var buildSignature = function(req, consumerSecret, tokenSecret){
  ...
  var url       = 'http://api.flickr.com' + req.path;

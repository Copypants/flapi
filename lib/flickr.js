...
exports.makeRequest = function(request, photo, callback){
  ...
  if(photo){
    var earl = 'https://' + reqOpts.host + reqOpts.path;
    var form = {};
    ...
    var data    = [];
    var httpReq = https.request(reqOpts, function(res){
      res.on('data',function(chunk){
        data.push(chunk);
      });

      res.on('end',function(){
        callback(null, data.join(''), res);
      });
    })
    .on('error', callback);

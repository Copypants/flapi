...
flickr.prototype.authApp = function(authCallbackURL, next){
  ...
  if(!settings.consumerKey)     throw new Error('A consumerKey (app key) is required. ' + message);
  if(!settings.consumerSecret)  throw new Error('A consumerSecret (app secret) is required. ' + message);
...
var addDefaultParams = function(obj){
  var defaults = {
...
// at least i think so
var buildSignature = function(opts){
  if(!opts.queryParams) opts.queryParams  = {};
  if(!opts.method) opts.method            = 'GET';
  if(!opts.secrest) opts.secret           = ''

  opts.queryParams = addDefaultParams(opts.queryParams);
...
var logger = function(msg){
  if(settings.logging) console.log(msg)
}

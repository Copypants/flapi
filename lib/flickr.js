...
Please pass an api method option as "method". You can find all available flickr methods within the flickr documentation at http://www.flickr.com/services/api/');
  ...
  for(var key in opts.params){
    if(key != 'photo') queryParams[key] = opts.params[key];
  }
...
var makeRequest = function(request, next, photo){
  ...
  if(photo){
    ...
    headers.host    = 'up.flickr.com';
    reqOpts.headers = headers;

    for(var key in request.queryParams){
      form.append(key, request.queryParams[key]);
    }
    form.append('photo', photo)

    var httpReq = http.request(reqOpts);
    form.pipe(httpReq);
    ...
    httpReq.on('response',function(res) {
      ...
      // Flickr returns XML instead of JSON when creating a new photo
      res.on('end',function(){
        var parsed = XMLParser.toJson(data.join(''));
        next(JSON.parse(parsed).rsp, res);
      });
    });
  } else {
    var data    = [];

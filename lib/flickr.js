...
flickr.prototype.getUserAuthURL = function(){
  ...
  return 'http://www.flickr.com/services/oauth/authorize?oauth_token=' + this.settings.oauth_token + perms
};
...
Please pass an api method option as "method". You can find all available flickr methods within the flickr documentation at http://www.flickr.com/services/api/');
  ...
  // Default object used to make all api requests
  var queryParams = {
    method          : opts.method,
    api_key         : this.settings.oauth_consumer_key,
    format          : 'json',
    nojsoncallback  : 1,
  ...
  // If this is a photo upload, make some slight adjustments to
  // the request object
  ...
  makeRequest(request, function(body, res){
    if(opts.next) opts.next(JSON.parse(body));
  });
};
...
var makeRequest = function(request, next){
  ...
  var reqOpts  = {
    host    : request.host || 'www.flickr.com',
    path    : request.path + query,
    method  : request.method
  };

  if(request.queryParams.photo){
    postData        = queryString.stringify(request.queryParams);
    reqOpts.headers = {
...
var buildSignature = function(req, consumerSecret, tokenSecret){
  ...
  var url       = 'http://www.flickr.com' + req.path;
